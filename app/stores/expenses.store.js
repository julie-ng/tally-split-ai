...
export const useExpensesStore = defineStore('expenses', () => {
   ...
   * @param {String} payload.expenseId - our nanoid
   */
  function ingestExpense (payload) {
    ...
    if (saving.value[expenseId]) {
      console.log(`📡 [Broadcast] ⏭ skipped ${expenseId} — save in flight (expected for your own edit)`)
      return
    }

    const existing = expenses.value[expenseId]

    // Ensure our expenseId (not payload id) is mapped to id
    // eslint-disable-next-line no-unused-vars
    const { id: _messageId, expenseId: _expenseId, ...fields } = payload

    expenses.value[expenseId] = { ...(existing ?? {}), ...fields, id: expenseId }

    // Keep the receiptId index in step. A standalone expense has no receiptId,
    // hence the guard — never index undefined.
    if (payload.receiptId) {
      receiptToExpense.value[payload.receiptId] = expenseId
    }

    console.log(`📡 [Broadcast] ✅ ${existing ? 'merged into' : 'ADDED NEW'} expense ${expenseId}`)
  }
