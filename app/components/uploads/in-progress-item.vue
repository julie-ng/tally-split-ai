  ...
  <article v-if="upload" class="my-4 p-5 bg-white border-slate-200 border-solid border rounded-md">
    ...
    <div v-if="upload.status === 'in-progress'" class="mb-3">
      <p class="text-sm text-blue-600 font-medium mb-1">
        Uploading: {{ upload.upload.progress }}%
      </p>
      <div class="w-full bg-slate-200 rounded-full h-2">
        <div class="bg-blue-500 h-2 rounded-full transition-all duration-300" :style="{ width: `${upload.upload.progress}%` }" />
      </div>
    </div>

    <div v-if="upload.status === 'failed'" class="mb-3 p-3 bg-rose-50 border border-rose-200 rounded">
      <p class="text-rose-700 text-base font-semibold mb-2">
        Upload Failed
      </p>
    ...
    <p class="mb-1 text-xs uppercase text-slate-400">
      Azure Blob Name
    </p>
