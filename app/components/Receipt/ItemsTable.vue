...
<table class="my-3 text-sm">
  <thead>
    <tr>
      <th v-if="hasQuantity" class="p-2 border-b border-slate-300 font-medium">Qty.</th>
      <th class="p-2 border-b border-slate-300 font-medium text-left">Item</th>
      <th class="p-2 border-b border-slate-300 font-medium text-right">Price</th>
    </tr>
  </thead>
  <tbody>
    ...
    <tr v-if="subtotal">
      ...
      <td :colspan="hasQuantity ? 1 : 2" class="p-2 pr-4 text-right">Subtotal</td>
      <td class="p-2 text-right">{{ receiptUtils.formatCurrency(subtotal) }}</td>
    </tr>
  </tbody>
</table>
</div>
