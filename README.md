...
- **Action-scoped HMAC tokens.** Each task gets only the `resource:permission` pairs it needs (e.g. `expense:write`, `upload:read`). Tasks cannot mint their own tokens; only registered orchestrators can, and only for their declared children.
