...
export function getGpt4oConfig () {
  const endpoint = process.env.AZURE_GPT4O_ENDPOINT
  const key = process.env.AZURE_GPT4O_KEY

  if (!endpoint) {
    throw new Error('AZURE_GPT4O_ENDPOINT environment variable is not set')
  }
