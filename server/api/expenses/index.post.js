...
import { expenseRequestSchema } from '#shared/utils/zod-schemas/expense.schema.js'
...
export default defineEventHandler(async (event) => {
  ...
  const result = await readValidatedBody(event, body => expenseRequestSchema.safeParse(body))
  if (!result.success) {
  ...
  if (result.data.receiptId) {
    const [existing] = await db
      ...
      .where(eq(schema.expenses.receiptId, result.data.receiptId))
      .limit(1)

    if (existing) {
      log.info({ expenseId: existing.id, receiptId: result.data.receiptId }, 'Expense already exists for receipt — returning existing')
  ...
  log.info({ expenseId: created.id }, 'Expense created')
